...
OUTPUT_FILE = (
    f"squiggles_results/GEMMA/direct{str(args.direct)}_dataset{args.dataset}_"
    f"model{args.model_path.replace('/', '_')}.jsonl"
)
os.makedirs(os.path.dirname(OUTPUT_FILE), exist_ok=True)


def collate_fn(batch, processor, model):
    messages_list = []
    answers_list = []
    shapes_list = []
    images_list = []
    ...
    inputs = processor(images_list, texts, padding=True, return_tensors="pt").to(
...
def main():
    ...
    processor = AutoProcessor.from_pretrained(
        args.model_path, trust_remote_code=True, padding_side='left', do_pan_and_scan=args.ps
    )
    ...
    _collate_fn = partial(collate_fn, processor=processor, model=model)
