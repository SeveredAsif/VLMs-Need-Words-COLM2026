...
class Config:
    ...
    num_workers = 4
    prefetch_factor = 2
    output_dir = "qwen_2b_finetuned_on_task"
...
def train_collate_fn(batch, processor):
    ...
    labels_list = []
    assistant_start_token_seq = [151644, 77091, 198]

    def get_prompt_length(input_ids):
        input_ids = input_ids.tolist()
        for i in range(len(input_ids) - len(assistant_start_token_seq) + 1):
            if input_ids[i:i+len(assistant_start_token_seq)] == assistant_start_token_seq:
                return i
        raise ValueError("Assistant start token sequence not found in input_ids")

    for i in range(len(batch)):
        prompt_length = get_prompt_length(inputs["input_ids"][i])
        label = inputs["input_ids"][i].clone()
        assistant_start_len = len(assistant_start_token_seq)
        label[:prompt_length + assistant_start_len] = -100
...
def evaluate(model, processor, test_dataloader, config):
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            batch_input = {k: v.to(config.device) if isinstance(v, torch.Tensor) else v
            ...
            for pred, answer in zip(predictions, ground_truth_answers):
                assistant_response = pred.split("<|im_end|>\n<|im_start|>assistant\n")[-1]
                assistant_response = assistant_response.split("<|im_end|>")[0].strip()
                if answer.strip() in assistant_response:
                    correct += 1
                total += 1
