...
OUTPUT_FILE = f"results/GEMMA/{args.filters}_direct{str(args.direct)}_model{args.model_path.replace('/', '_')}.jsonl"
TEST_DATASET = "test"
...
if __name__ == "__main__":
    ...
    processor = AutoProcessor.from_pretrained(BASE_MODEL_PATH, trust_remote_code=True, padding_side='left', do_pan_and_scan=PAN_AND_SCAN)
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            responses = [parse_gemma_response(pred) for pred in preds]
    ...
    print(f"\nFilter Conditions: {str(FILTER_CONDITIONS)} Direct?: {args.direct} Model: {args.model_path} Accuracy: {correct}/{total} = {100*correct/total:.2f}%\n")
